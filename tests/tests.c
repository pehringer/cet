...
int capacity_one(void) {
    int i;
    const int *p;
    set_t *s = set_Create(1, sizeof(int), intHash, intCompare);
    if(s == 0) return 1;
    if(set_Capacity(s) != 1) return 2;
    if(set_Length(s) != 0) return 3;
    if(set_Iterate(s, 0) != 0) return 4;
    i = 21;
    if((p = set_Insert(s, &i)) == 0 | *p != i) return 5;
    i = 42;
    if(set_Insert(s, &i) != 0) return 6;
    i = 21;
    if((p = set_Contains(s, &i)) != set_Insert(s, &i) || *p != i) return 7;
    i = 42;
    if(set_Contains(s, &i) != 0) return 8;
    if(set_Capacity(s) != 1) return 9;
    if(set_Length(s) != 1) return 10;
    if((p = set_Iterate(s, 0)) == 0 || *p != 21) return 11;
    if(set_Iterate(s, p) != 0) return 12;
    i = 21;
    set_Remove(s, &i);
    i = 42;
    set_Remove(s, &i);
    ...
    i = 42;
    if(set_Contains(s, &i) != 0) return 14;
    if(set_Capacity(s) != 1) return 15;
    if(set_Length(s) != 0) return 16;
    if(set_Iterate(s, 0) != 0) return 17;
    set_Destroy(s);
    return 0;
}
